...
* and a transform definition (explained later), which can also be sourced from the payload, metadata or a file.
* The transform engine, powered by JOLT ({@link https://github.com/bazaarvoice/jolt}), will take your source JSON content and apply the transform definition,
* the result of which will be more JSON content, which can be saved into payload, metadata or even a file.
* </p>
* <p>
...
* </p>
* <p>
* Using the same destination options above you can also set the destination for the result of the transform by setting "target-result-destination". 
* </p>
* <p>
* The transform definition is called "Shitfr", explained below, but is generally a static block of JSON content.  Although parameters can not be passed into the
* transform engine, we can apply relatively simple variable substitution on the Shiftr content during service execution.  To do this simply set "apply-metadata-as-parameters"
* to "true".  In this case, all metadata in the currently processing message will be used to inject data into variable place-holders in the Shiftr content.
* <br/>
* All metadata key names surrounded by "${<key-name>}" will be searched through the Shiftr JSON content and replaced by the metadata value for that key.
* </p>
* <p>
* An example:
* <br/>
...
public class JsonTransformService extends ServiceImp {
  ...
  private Boolean applyMetadataAsParameters;
  ...
  private String applyMetadataSubstitution(AdaptrisMessage message, String shiftrContent) {
    if(applyMetadataAsParameters()) {
      for(MetadataElement element : message.getMetadata()) {
  ...
  public Boolean applyMetadataAsParameters() {
    return this.isApplyMetadataAsParameters() != null ? this.isApplyMetadataAsParameters() : false;
  }
  
  public Boolean isApplyMetadataAsParameters() {
    return applyMetadataAsParameters;
  }

  public void setApplyMetadataAsParameters(Boolean applyMetadataAsParameters) {
    this.applyMetadataAsParameters = applyMetadataAsParameters;
  }
  ...
  public void setTargetResultDestination(DataDestination targetResultDestination) {
    this.targetResultDestination = targetResultDestination;
  }
  ...
  public void setSourceJsonDestination(DataDestination sourceJsonDestination) {
    this.sourceJsonDestination = sourceJsonDestination;
  }
  ...
  public void setSourceSpecDestination(DataDestination sourceSpecDestination) {
    this.sourceSpecDestination = sourceSpecDestination;
  }
