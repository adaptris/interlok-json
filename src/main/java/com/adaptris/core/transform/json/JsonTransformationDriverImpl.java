...
public abstract class JsonTransformationDriverImpl implements TransformationDriver {
  ...
  /*
   * These fields exists to make XStream serialize the driver properly. Since the driver uses an XMLSerializer instance, exposing
   * only SOME of its settings, we don't want the serializer itself to be generated into the XML. There are some issues with the
   ...
   * Convert XML to JSON.
   *
   * @param input The XML to convert.
   *
   * @return The converted JSON.
   *
   * @throws ServiceException Thrown if there was a problem converting from XML to JSON.
   */
   ...
   * Convert JSON to XML.
   *
   * @param input The JSON to convert.
   *
   * @return The converted XML.
   *
   * @throws ServiceException Thrown if there was a problem converting from JSON to XML.
   */
   ...
   * @return The JSON.
   *
   * @throws JSONException Thrown if the string could not be parsed to JSON.
   */
   ...
   */
  String arrayName() {
    return ObjectUtils.defaultIfNull(getArrayName(), DEFAULT_ARRAYNAME);
  }
   ...
   */
  String elementName() {
    return ObjectUtils.defaultIfNull(getElementName(), DEFAULT_ELEMENTNAME);
  }
  ...
  public String objectName() {
    return ObjectUtils.defaultIfNull(getObjectName(), DEFAULT_OBJECTNAME);
  }
   ...
   */
  String rootName() {
    return ObjectUtils.defaultIfNull(getRootName(), DEFAULT_ROOTNAME);
  }
  ...
  boolean forceTopLevelObject() {
    return BooleanUtils.toBooleanDefaultIfNull(getForceTopLevelObject(), DEFAULT_FORCE_TOP_LEVEL_OBJECT);
  }

  /**
   * Set whether to force top level object.
   *
   * @param forceTopLevelObject Whether to force top level object.
   */
  ...
  boolean skipWhitespace() {
    return BooleanUtils.toBooleanDefaultIfNull(getSkipWhitespace(), DEFAULT_SKIP_WHITE_SPACE);
  }
  ...
  boolean trimSpaces() {
    return BooleanUtils.toBooleanDefaultIfNull(getTrimSpaces(), DEFAULT_TRIM_SPACES);
  }

  /**
   * Set whether to trim whitespace.
   *
   * @param trimSpaces Whether to trim whitespace.
   */
  ...
  boolean typeHintsCompatibility() {
    return BooleanUtils.toBooleanDefaultIfNull(getTypeHintsCompatibility(), DEFAULT_TYPE_HINTS_COMPAT);
  }

  /**
   * Set whether type hints compatibility is enabled.
   *
   * @param typeHintsCompatibility Whether type hints compatibility is enabled.
   */
  ...
  boolean typeHintsEnabled() {
    return BooleanUtils.toBooleanDefaultIfNull(getTypeHintsEnabled(), DEFAULT_TYPE_HINTS_ENABLED);
  }

  /**
   * Set whether type hints is enabled.
   *
   * @param typeHintsEnabled Whether type hints is enabled.
   */
