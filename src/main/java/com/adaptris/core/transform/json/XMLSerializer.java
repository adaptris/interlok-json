...
class XMLSerializer {
  ...
  private static final String INVALID_XOM_ELEMENT_CHAR = "[^\\w]";
   ...
   * @return a String representation of a well-formed xml document.
   * @throws JSONException if the conversion from JSON to XML can't be made for I/O reasons or the
  ...
  public String write(JSON json, String encoding) {
    if (JSONNull.getInstance().equals(json)) {
      ...
      Element root =
          processJSONArray(jsonArray, newElement(getRootName() == null ? getArrayName() : getRootName()), expandableProperties);
      Document doc = new Document(root);
      return writeDocument(doc, encoding);
    } else {
      ...
      if (jsonObject.isNullObject()) {
        ...
        root =
            processJSONObject(jsonObject, newElement(getRootName() == null ? getObjectName() : getRootName()),
  ...
  private boolean isFunction(Element element) {
    ...
    if (attrCount > 0) {
      ...
      if (attrCount == 2
          && paramsAttr != null
          && typeAttr != null
          && (typeAttr.getValue().compareToIgnoreCase(JSONTypes.STRING) == 0 || typeAttr.getValue().compareToIgnoreCase(
              JSONTypes.FUNCTION) == 0)) {
        return true;
      }
    }
    return false;
  ...
  private Element newElement(String name) {
    ...
    return namespaceLenient ? new CustomElement(name) : new Element(safeElementName(name));
  }
  ...
  private JSON processObjectElement(Element element, String defaultType) {
    ...
    for (int i = 0; i < attrCount; i++) {
      ...
      if (isTypeHintsEnabled()
          && (addJsonPrefix("class").compareToIgnoreCase(attrname) == 0 || addJsonPrefix("type").compareToIgnoreCase(attrname) == 0)) {
        continue;
      }
  ...
  // Creates a safe element name for xom Elements.
  // $type -> _type
  private static String safeElementName(String input) {
    ...
    // xom seems to be somewhat stricter than XML 1.0; so let's change all non-words into _
    name = name.replaceAll(INVALID_XOM_ELEMENT_CHAR, ELEM_REPL_VALUE);
    return name;
  }
  ...
  private static class CustomElement extends Element {
    private static String getName(String name) {
      int colon = name.indexOf(':');
      if (colon != -1) {
        return safeElementName(name.substring(colon + 1));
      }
      return name;
    }
    ...
    private static String getPrefix(String name) {
      int colon = name.indexOf(':');
      if (colon != -1) {
        return safeElementName(name.substring(0, colon));
      }
      return "";
    }
